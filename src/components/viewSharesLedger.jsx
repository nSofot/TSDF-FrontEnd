...
export default function ViewSharesLedger({ customerId }) {
    ...
    const { sharesLedger, loading, error } = useSharesLedger(customerId);

    if (!customerId) {
      return <p className="text-gray-500 px-4 py-2">No account selected</p>;
    }

    if (loading) return <p className="text-gray-500 px-4 py-2">Loading shares ledger...</p>;
    if (error) return <p className="text-red-500 px-4 py-2">{error}</p>;
    
    return (
        <div className="w-full h-full text-sm text-gray-700 rounded shadow">
